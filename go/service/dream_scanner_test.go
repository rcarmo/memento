...
func TestDetectDreamSignals(t *testing.T) {
	...
	config := DreamScannerConfig{OversizedBodyChars: 10, OversizedTopLevelSections: 2, MaxOversizedCandidates: 2}
...
func TestDreamOversizedTie(t *testing.T) {
	...
	signals := DetectDreamSignals(bundle, "r", "", nil, DreamScannerConfig{OversizedBodyChars: 5, OversizedTopLevelSections: 9, MaxOversizedCandidates: 1})
	found := ""
	for _, signal := range signals {
