...
`propose` creates a new `submitted` record immediately. Inspect `proposal_list(status="unresolved")` for the review queue, including draft, submitted, approved, needs-rebase and conflicted records; `pending` is not a status. `request_changes` sets `draft`, and `reject` sets `rejected`. Read `review_comment`, correct the local content and file a new proposal at the current revision, citing the old proposal ID in `rationale`. There is no edit/resubmit API for an existing draft. `propose_update` drafts a concept change from an instruction; it does not edit a stored proposal.

After repository advancement, clean old-base proposals become visible `needs_rebase`; overlapping changes become visible `conflicted`. Curators can reject or request changes in either state. The original proposer or an authorised curator can use execute-only `proposal_rebase(proposal_id, expected_revision, idempotency_key)` on a clean proposal: it retains the same ID, author, assets and creation time, clears approval into history, and returns `submitted` for fresh review. Reconcile a lost response with the original key through `operation_get`. For a clean subset, curator-only `proposal_revise` creates a new proposal and copies paired concept/assets. Conflicting content, expired records and archival with an old impact report require fresh preparation and submission. A corrected packaged skill needs revised asset bytes as well as Markdown; inspect its manifest and root `SKILL.md` parity before approval.
...
Publish packs that fit the configured MCP request ceiling with an `attach_asset_pack` change containing `zip_base64`. This keeps the complete proposal inside MCP. Read the created proposal and verify its generated manifest, SHA-256, target path, asset kind and version before review. After apply, inspect `memory_asset_get(view="manifest")` and retrieve files with `view="file", file_path=...`, or download archive chunks. Resume with the returned `version`, ZIP digest as `expected_sha256`, and `next_offset` until null; verify the assembled ZIP digest before installing. Do not assume every `zip_base64` contains a complete archive. `memory_status.limits.assets` exposes byte/count limits. Memento stores and returns skill packs; it does not execute them.
