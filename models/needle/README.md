...
The deterministic splits were generated by the removed Python training harness; the recipe and hard-negative continuation are preserved in ADR 0002 and the retained evidence files. The historical Rust runtime and current pure-Go scalar/SIMD runtime match the passing checkpoint on all 360 held-out cases. The Go implementation also matches all 360 outputs/errors under NEON on `orangepi6plus.local`; scalar, NEON and automatic dispatch pass the five real generation cases. Go timings and the ARM64 reduction fix are recorded in [`docs/evidence/go-real-model-simd-2026-09-19.json`](../../docs/evidence/go-real-model-simd-2026-09-19.json).
