...
The deterministic splits come from `tools/experiments/needle/generate_router_v2.py`; the hard-negative continuation is recorded in ADR 0002 and `router-v2-training-summary.json`. The Rust runtime and pure-Go scalar/SIMD runtime match the passing checkpoint on all 360 held-out cases. The Go implementation also matches all 360 outputs/errors under NEON on `orangepi6plus.local`; scalar, NEON and automatic dispatch pass the five real generation cases. The C ABI and Python wrapper retain bounded output, lifecycle checks and cooperative cancellation for the released Python service. Go timings and the ARM64 reduction fix are recorded in [`docs/evidence/go-real-model-simd-2026-09-19.json`](../../docs/evidence/go-real-model-simd-2026-09-19.json).
