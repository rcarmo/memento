...
* Semantic model path overrides are optional. `MEMENTO_GTE_MODEL`, `MEMENTO_NEEDLE_MODEL`, `MEMENTO_NEEDLE_FP32_MODEL`, `MEMENTO_NEEDLE_TOKENIZER` and `MEMENTO_NEEDLE_WORKER` override the release defaults. The Go runtime defaults both model paths to subprocess mode. Keep shared rollback-compatible JSON free of the newer worker/FP32 keys because `0.5.9` rejects unknown fields; use the environment overrides instead. Legacy FFI and SQLite-extension fields remain accepted in schema-version-2 JSON but the Go runtime does not load them.
