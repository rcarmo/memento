...
| [0009](0009-run-gte-in-batched-short-lived-workers.md) | Run GTE in short-lived workers; `v1.0.0` preserves process isolation with the pure-Go worker | accepted, amended by Go replacement |
| [0010](0010-use-tiled-matrix-kernels-for-local-inference.md) | Optimise local inference without changing model outputs; `v1.0.0` uses pure-Go SIMD projection kernels and reusable workspaces | accepted, amended by Go replacement |
