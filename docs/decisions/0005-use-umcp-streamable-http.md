...
> `v1.0.0` keeps the protocol decision and implements the pinned observable uMCP behaviour in the standalone `go/umcp` module. The external Python package is no longer a runtime dependency.

## Decision

Memento uses the standalone pure-Go `go/umcp` module as its MCP server and transport core, pinned behaviourally to [`rcarmo/umcp`](https://github.com/rcarmo/umcp). Network clients connect through Streamable HTTP with bearer authentication. Caller identity comes from uMCP's request context, never from tool arguments.
...
* Transport upgrades are reviewed against the pinned reference and implemented in `go/umcp`; the release image contains no external uMCP package.
* The module path is `github.com/rcarmo/memento/go/umcp`, avoiding the unrelated `umcp` package name on PyPI.
