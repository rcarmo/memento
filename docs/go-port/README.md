...
The Git store can read existing packed SHA-1 objects/refs and compare tree paths without a Git subprocess. Main publication uses an exclusive Git-compatible lock file, a locked revision recheck and atomic rename, with failures after publication distinguished from failures before it. Bootstrap, worktree commits, materialisation and recovery are the next storage steps; reflogs and symbolic main refs fail explicitly for now.
