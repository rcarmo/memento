...
`go/repository` now ports canonical path validation, reserved-file classification, concept metadata validation and JSON/YAML frontmatter. Differential tests preserve UTC conversion, source coercion quirks, body normalisation and deterministic YAML output for the tested cases. Filesystem checks do not authorise access or make later opens race-safe; broader YAML compatibility, links, transactions and service operations still need implementation.
