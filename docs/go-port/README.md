...
`go/repository` now ports canonical path validation, reserved-file classification and filesystem checks against the unchanged Python baseline. Those checks do not authorise access or make later opens race-safe; repository transactions and service operations still need implementation.
