...
The Go audit includes formatting/layout, vet, pinned Staticcheck, tests, a zero-uncovered-statement gate, govulncheck, race detection, all 54 fuzz targets at 10,000 executions each and Linux amd64/arm64 cross-builds. Release checks build reproducible static archives for the daemon, embedding worker and skill importer, then validate layout, checksums, ELF metadata and native smoke tests.
