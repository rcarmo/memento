...
`make -C go model-test` now requires both `GTE_MODEL_PATH` and `NEEDLE_MODEL_PATH`; the Needle fixture validates the 31 tensor shapes/raw bytes/expanded FP32 hashes and all 8192 vocabulary entries from the real NDL1 model. Regenerate with `cargo run --locked --release --manifest-path go/oracle/rust-needle/Cargo.toml -- /path/to/memento-router.ndl`. Add `NEEDLE_TOKENIZER_PATH` for the now-required tokenizer gate: 15 encode/decode inputs and all 8192 single-token decodes are compared with Rust. Always-on tests compare 288 synthetic BPE/Unigram normalisation/byte-fallback cases against sentencepiece-rust 0.1.1. This proves those parser/tokenizer cases, not Needle inference or a complete tokenizer corpus. Source and port reject malformed files, but their JSON/UTF-8 diagnostic wording has not yet been made fully identical.
