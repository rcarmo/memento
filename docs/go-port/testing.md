...
* Legacy SSE compares 68 live sync/async requests plus endpoint handshakes, selected headers and JSON events. Tests cover principal/session binding, trusted subscription IDs and cleanup, fresh IDs on reconnect, targeted/broadcast notifications, unbounded sync queues, async write acknowledgement, keepalive, failed writes/flushes and session rechecks. Sync and async parser-order differences are explicit. The sync unsupported-method HTML body belongs to the unfinished raw-server gate; its status is compared here. Notification stdout fallback and transport CLI integration are also unfinished.
