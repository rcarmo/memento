...
Each JSON document records the Git revision available when the harness started, host and Python information, operation counts, throughput, p50/p95/p99/max latency, errors, invariants and threshold decisions. A report only passes when every included scenario and invariant passes.
