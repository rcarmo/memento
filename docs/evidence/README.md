...
[`release-0.3.26.md`](release-0.3.26.md) records the current immutable release identity, pinned uMCP dependency, persistent Streamable HTTP checks and state-preserving DiskStation deployment.

[`release-0.3.25.md`](release-0.3.25.md) records the previous embedding-preserving deployment, live graph and semantic state, and execute-only asset metadata checks. [`release-0.3.24.md`](release-0.3.24.md) records the earlier bounded inventory and manifest comparison deployment, including progressive embedding convergence. [`release-0.3.23.md`](release-0.3.23.md) remains the historical answer, redeployment and tagged-source corpus record. All four reports distinguish release gates from checks that ran against the live service.
