...
The repository, transaction, proposal, indexing, MCP, model, access-management, debugger, backup and restore paths are covered by strict 100% statement coverage, race checks and package-wide fuzzing. CI runs the complete model-independent Go audit, native amd64/ARM64 real-model and allocation gates, the 360-case Needle corpus on ARM64, and the replacement-container contract on amd64. Release tags validate reproducible static archives, then publish native multi-architecture images and an SPDX JSON SBOM after non-cancelling validation and a Westmere no-AVX smoke.

The operator-managed DiskStation deployment serves authenticated MCP clients from the immutable pure-Go `v1.0.0` image, persists Git/control/derived state, exposes the trusted-LAN graph debugger and has exercised distinct managed principals, namespace filtering, curator proposal review/apply (including same-principal authorship), asset publication/retrieval and container replacement. The live replacement passed target backup, state/revision continuity, semantic and Needle subprocess checks, restart acceptance and the 512 MiB cap; [`docs/evidence/release-1.0.0.md`](evidence/release-1.0.0.md) records the result. Published Go releases carry immutable OCI digests, BuildKit provenance attestations and an SPDX JSON SBOM.
