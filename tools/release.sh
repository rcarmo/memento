...
for ARCH in amd64 arm64; do
  ...
  env CGO_ENABLED=0 GOOS=linux GOARCH="$ARCH" $AMD "$GO" build -trimpath -buildvcs=false -ldflags="-s -w -X 'main.version=$VERSION' -X 'github.com/rcarmo/memento/internal/service.BuildVersion=$VERSION'" -o "$STAGE/memento-go" ./cmd/memento-go
