...
const nodes = Array.from({length: Math.min(count, 2000)}, (_, i) => {const type=types[i%types.length],namespace=namespaceFor(type);return{
  id:`node-${i}`, path:`${namespace}node-${i}.md`, title:`Memory ${i}`, type, status:"active", tags:["fixture",`group-${i%12}`], namespace, updated_at:"2026-07-20T00:00:00Z", updated_by:i%3?"piclaw":"curator", markdown_bytes:200+i*3, asset_bytes:i%8===0?5000:0, combined_bytes:200+i*3+(i%8===0?5000:0), explicit_in_degree:i%9, explicit_out_degree:(i*3)%11, broken_link_count:i%97===0?1:0, orphan:i%113===0, proposal_count:i%17===0?1:0, pending_proposal_count:i%53===0?1:0, embedding:{status:i%19===0?"missing":"ready",model_id:"gte-small",dimensions:384,embedding_revision:"fixture-rev"}, coarse_position:{x:Math.cos(i*.41)*(2+Math.sqrt(i)*.18),y:Math.sin(i*.41)*(2+Math.sqrt(i)*.18),z:((i%17)-8)*.12}, anomaly_ids:i%97===0?[`diagnostic:broken:${i}`]:[]
};});
const explicitEdges = Array.from({length: Math.max(0,nodes.length-1)},(_,i)=>({id:`edge-${i}`,source:`node-${i}`,target:`node-${i+1}`,raw_target:`/projects/node-${i+1}.md`,kind:"explicit",canonical:true,resolution:"resolved",first_seen_revision:"fixture-rev",last_checked_revision:"fixture-rev"}));
const semanticEdges = Array.from({length: Math.min(nodes.length,60)},(_,i)=>({id:`semantic-${i}`,source:`node-${i}`,target:`node-${(i+7)%nodes.length}`,raw_target:`cosine:${(.82+(i%12)*.012).toFixed(3)}`,kind:"semantic_similarity",canonical:false,resolution:"derived",first_seen_revision:"fixture-rev",last_checked_revision:"fixture-rev",similarity:.82+(i%12)*.012,model_id:"gte-small",embedding_revision:"fixture-rev"}));
const edges=[...explicitEdges,...semanticEdges];
