...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`Invalid graph response (${response.status})`);
  }
...
export const graphApi = {
  ...
  refresh: (scope, conceptIds = [], confirmFull = false) =>
    request("/embeddings/refresh", {
      method: "POST",
      body: JSON.stringify({ scope, concept_ids: conceptIds, confirm_full: confirmFull }),
    }),
  exportSvg: (conceptIds, settings = {}) =>
    download("/export/svg", { concept_ids: conceptIds, settings }),
  exportJson: (conceptIds, settings = {}) =>
    download("/export/json", { concept_ids: conceptIds, settings }),
};
